...
export function makeDB(sb) {
  return {
    ...
    async getTrackingSummary() {
      ...
      // 3. Lấy TẤT CẢ events cho tất cả resend_ids (1-2 queries max)
      const allResendIds = allLogs.map(l => l.resend_id).filter(Boolean);
      let allEvents = [];
      const EV_BATCH = 200;
      for (let i = 0; i < allResendIds.length; i += EV_BATCH) {
        const batch = allResendIds.slice(i, i + EV_BATCH);
        const { data: events } = await sb.from('email_events')
          .select('event_type, recipient_email, resend_email_id')
          .in('resend_email_id', batch);
        allEvents = allEvents.concat(events || []);
      }
