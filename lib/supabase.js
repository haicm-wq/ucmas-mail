...
export function makeDB(sb) {
  return {
    ...
    async getCampaignEvents(campaignId) {
      // Get all send_logs for this campaign, then find their events
      ...
      // Batch fetch events (Supabase .in() max ~300 items, so batch)
      let allEvents = [];
      const BATCH = 200;
      for (let i = 0; i < resendIds.length; i += BATCH) {
        const batch = resendIds.slice(i, i + BATCH);
        const { data: events, error: evErr } = await sb.from('email_events')
          .select('*')
    ...
    async getCampaignTrackingStats(campaignId) {
      ...
      const bounced = allEvents.filter(e => e.event_type === 'bounced').length;
      const complained = allEvents.filter(e => e.event_type === 'complained').length;

      // Unique (by email)
      const uniqueOpens = new Set(allEvents.filter(e => e.event_type === 'opened').map(e => e.recipient_email)).size;
      const uniqueClicks = new Set(allEvents.filter(e => e.event_type === 'clicked').map(e => e.recipient_email)).size;

      const base = totalSent || 1;
      return {
        total_sent: totalSent, total_failed: totalFailed,
        delivered, opened, clicked, bounced, complained,
        unique_opens: uniqueOpens, unique_clicks: uniqueClicks,
        open_rate: Math.round((uniqueOpens / base) * 100),
        click_rate: Math.round((uniqueClicks / base) * 100),
      };
    },

    async getTrackingSummary() {
      // Get recent campaigns with tracking data
      const { data: campaigns, error: campErr } = await sb.from('campaigns')
        .select('id, name, status, sent_count, failed_count, sent_at')
        .order('created_at', { ascending: false })
        .limit(20);
      if (campErr) throw campErr;

      // For each campaign, get basic event counts
      const results = [];
      for (const camp of (campaigns || [])) {
        ...
        const resendIds = (logs || []).map(l => l.resend_id).filter(Boolean);
        let stats = { delivered: 0, opened: 0, clicked: 0, bounced: 0, complained: 0 };

        if (resendIds.length > 0) {
          const { data: events } = await sb.from('email_events')
            .select('event_type, recipient_email')
            .in('resend_email_id', resendIds.slice(0, 200));

          if (events) {
            stats.delivered = events.filter(e => e.event_type === 'delivered').length;
            stats.opened = new Set(events.filter(e => e.event_type === 'opened').map(e => e.recipient_email)).size;
            stats.clicked = new Set(events.filter(e => e.event_type === 'clicked').map(e => e.recipient_email)).size;
            stats.bounced = events.filter(e => e.event_type === 'bounced').length;
            stats.complained = events.filter(e => e.event_type === 'complained').length;
          }
        }

        results.push({ ...camp, tracking: stats });
      }
      return results;
    },
