...
export function makeDB(sb) {
  return {
    /** Expose raw supabase client cho direct queries */
    _sb() { return sb; },
    ...
    // Đếm contacts theo level — ưu tiên SQL function v2 (dùng level_ids)
    async getContactCountsPerLevel() {
      // Thử v2 trước (multi-level)
      try {
        const { data, error } = await sb.rpc('count_contacts_per_level_v2');
        if (!error && data) {
          const countMap = {};
      ...
      // Fallback JS: dùng level_ids nếu có, level_id nếu không
      const countMap = {};
      let offset = 0;
      const BATCH = 1000;
      while (true) {
        const { data, error } = await sb.from('contacts')
          .select('level_id, level_ids')
          .eq('status', 'active')
          .range(offset, offset + BATCH - 1);
        if (error) throw error;
        if (!data || data.length === 0) break;
        data.forEach(c => {
          const ids = c.level_ids?.length ? c.level_ids : (c.level_id ? [c.level_id] : []);
    ...
    async getContactsPaged({ levelId, search, status, tags, tagMode, offset = 0, limit = 200 } = {}) {
      ...
      if (levelId) base = base.contains('level_ids', [levelId]);
    ...
    // Trả về chỉ IDs của tất cả contacts khớp filter — dùng cho "chọn tất cả theo bộ lọc"
    async getFilteredContactIds({ levelId, search, status, tags, tagMode } = {}) {
      let base = sb.from('contacts').select('id');
      if (levelId) base = base.contains('level_ids', [levelId]);
      if (status)  base = base.eq('status', status);
      if (search)  base = base.or(`name.ilike.%${search}%,email.ilike.%${search}%`);
      if (tags?.length) {
        if (tagMode === 'or') base = base.overlaps('tags', tags);
        else base = base.contains('tags', tags);
      }
      // Batch fetch tất cả IDs
      const BATCH = 1000;
      let all = [], offset = 0;
      while (true) {
    ...
    /** Đếm contacts theo level IDs — nhẹ, chỉ lấy count */
    async countContactsByLevelIds(levelIds) {
      if (!levelIds?.length) return 0;
      const { count, error } = await sb.from('contacts')
    ...
    async upsertContacts(rows) {
      // Loại bỏ email trùng lặp trong cùng 1 batch (giữ bản cuối cùng)
      const seen = new Map();
      for (const row of rows) {
        if (row.email) seen.set(row.email.toLowerCase().trim(), row);
      }
      const unique = [...seen.values()];
      if (!unique.length) return [];
      
      // Multi-level: kiểm tra contacts đã tồn tại để append level
      const emails = unique.map(r => r.email.toLowerCase().trim());
      const { data: existing } = await sb.from('contacts')
        .select('id, email, level_id, level_ids')
        .in('email', emails);
      const existMap = new Map();
      (existing || []).forEach(c => existMap.set(c.email.toLowerCase(), c));
      
      const toInsert = [];
      const toUpdate = [];
      
      for (const row of unique) {
        const ex = existMap.get(row.email.toLowerCase());
        if (ex) {
          // Contact đã tồn tại → append level mới vào level_ids
          const currentIds = ex.level_ids?.length ? [...ex.level_ids] : (ex.level_id ? [ex.level_id] : []);
          if (row.level_id && !currentIds.includes(row.level_id)) {
            currentIds.push(row.level_id);
          }
          toUpdate.push({ id: ex.id, level_ids: currentIds, name: row.name || undefined, company: row.company || undefined, child_name: row.child_name || undefined });
        } else {
          // Contact mới → set level_ids = [level_id]
          row.level_ids = row.level_id ? [row.level_id] : [];
          toInsert.push(row);
        }
      }
      ...
      if (toInsert.length) {
        const { data, error } = await sb.from('contacts')
          .upsert(toInsert, { onConflict: 'email' }).select();
        if (error) throw error;
        inserted = data || [];
      }
      
      // Update contacts cũ (append levels)
      for (const upd of toUpdate) {
        const updateData = { level_ids: upd.level_ids };
        if (upd.name) updateData.name = upd.name;
        if (upd.company) updateData.company = upd.company;
        if (upd.child_name) updateData.child_name = upd.child_name;
        await sb.from('contacts').update(updateData).eq('id', upd.id);
      }
      
      return [...inserted, ...toUpdate];
    },
    async updateContactLevel(contactId, levelId) {
      // Multi-level: append level mới vào level_ids
      const { data: contact } = await sb.from('contacts')
        .select('level_id, level_ids').eq('id', contactId).single();
      const currentIds = contact?.level_ids?.length ? [...contact.level_ids] : (contact?.level_id ? [contact.level_id] : []);
      if (!currentIds.includes(levelId)) currentIds.push(levelId);
      const { error } = await sb.from('contacts')
        .update({ level_id: levelId, level_ids: currentIds }).eq('id', contactId);
      if (error) throw error;
    },
    ...
    async bulkDeleteContacts(ids) {
      // Xóa hàng loạt theo batch 200 — tránh vượt giới hạn query
      const BATCH = 200;
      let deleted = 0;
      for (let i = 0; i < ids.length; i += BATCH) {
        const batch = ids.slice(i, i + BATCH);
        const { error } = await sb.from('contacts').delete().in('id', batch);
        if (error) throw error;
    ...
    async getContactsBySegment({ levelIds, tags, tagMode, logic }) {
      ...
      if (logic === 'or') {
        ...
        const conditions = [];
        if (levelIds?.length) conditions.push(`level_ids.ov.{${levelIds.join(',')}}`);
        if (tags?.length) {
        ...
        if (levelIds?.length) base = base.overlaps('level_ids', levelIds);
        if (tags?.length) {
    ...
    // ── CAMPAIGNS ───────────────────────────────────
    async getCampaigns() {
      const { data: campaigns, error } = await sb.from('campaigns').select('*')
        .neq('name', '__KILL_SWITCH__')
        .order('created_at', { ascending: false });
      if (error) throw error;
      if (!campaigns?.length) return [];

      // Enriching: đếm sent/failed thực tế từ send_logs (fix sent_count=0 do mất kết nối)
      const campIds = campaigns.map(c => c.id);
      let allLogs = [];
      for (let i = 0; i < campIds.length; i += 500) {
        const batch = campIds.slice(i, i + 500);
      ...
      for (const log of allLogs) {
        if (!countMap[log.campaign_id]) countMap[log.campaign_id] = { sent: 0, failed: 0 };
        if (log.status === 'sent') countMap[log.campaign_id].sent++;
        else countMap[log.campaign_id].failed++;
      }

      // Fix counts + auto-complete campaigns bị kẹt
      for (const camp of campaigns) {
        const c = countMap[camp.id];
        if (!c) continue;
        const needsUpdate = c.sent > (camp.sent_count || 0) || c.failed > (camp.failed_count || 0);
        if (c.sent > (camp.sent_count || 0)) camp.sent_count = c.sent;
        if (c.failed > (camp.failed_count || 0)) camp.failed_count = c.failed;

        // Auto-complete: campaign paused/sending nhưng thực tế đã gửi hết
        if ((camp.status === 'paused' || camp.status === 'sending') && c.sent > 0) {
          try {
            const total = await this.countContactsByLevelIds(camp.target_levels || []);
            if (total > 0 && (c.sent + c.failed) >= total) {
              camp.status = 'completed';
              await this.updateCampaignStatus(camp.id, { status: 'completed', sent_count: c.sent, failed_count: c.failed }).catch(() => {});
              needsUpdate && console.log(`[getCampaigns auto-complete] ${camp.name}: ${c.sent}/${total}`);
              continue;
            }
          } catch (_) {}
        }
        // Persist sent_count nếu khác biệt
        if (needsUpdate) {
          await this.updateCampaignStatus(camp.id, { status: camp.status, sent_count: c.sent, failed_count: c.failed }).catch(() => {});
        }
      }
      return campaigns;
    },
    ...
    async updateCampaignStatus(id, updates) {
      const row = { status: updates.status, sent_at: new Date().toISOString() };
      if (updates.sent_count  >= 0) row.sent_count  = updates.sent_count;
      if (updates.failed_count >= 0) row.failed_count = updates.failed_count;
      const { error } = await sb.from('campaigns').update(row).eq('id', id);
      if (error) throw error;
    },

    /** Lấy status hiện tại của campaign — dùng để kiểm tra giữa vòng gửi */
    ...
    /** Dừng TẤT CẢ campaigns đang sending — trả về số lượng bị pause */
    async pauseAllSendingCampaigns() {
      const { data, error } = await sb.from('campaigns')
        .update({ status: 'paused' })
        .eq('status', 'sending')   // chỉ dừng các campaign đang gửi
        .select('id');
      if (error) throw error;
      return (data || []).length;
    },

    /** Lưu trạng thái kill switch vào 1 row đặc biệt trong bảng campaigns */
    async setKillSwitch(active) {
      const name = '__KILL_SWITCH__';
      const { data: existing } = await sb.from('campaigns')
        .select('id').eq('name', name).maybeSingle();
      if (existing) {
        await sb.from('campaigns').update({ status: active ? 'paused' : 'completed' }).eq('id', existing.id);
      } else {
        await sb.from('campaigns').insert({
          name,
          status:        active ? 'paused' : 'completed',
          subject:       '__internal__',
          body_text:     '__internal__',
          target_levels: [],
        });
      }
    },

    /** Đọc trạng thái kill switch — true = đang bị khoá */
    async getKillSwitch() {
      const { data } = await sb.from('campaigns')
        .select('status').eq('name', '__KILL_SWITCH__').maybeSingle();
    ...
    async getTrackingSummary() {
      // 1. Lấy campaigns (1 query) — loại bỏ row nội bộ
      const { data: campaigns, error: campErr } = await sb.from('campaigns')
        ...
        .neq('name', '__KILL_SWITCH__')          // ẩn row kill switch
        .order('created_at', { ascending: false })
        .limit(20);
      ...
      // 2. Lấy TẤT CẢ send_logs kèm status (để đếm chính xác)
      let allLogs = [];
      const LOG_BATCH = 500;
      for (let i = 0; i < campIds.length; i += LOG_BATCH) {
        const batch = campIds.slice(i, i + LOG_BATCH);
        const { data: logs } = await sb.from('send_logs')
          .select('campaign_id, resend_id, status')
          .in('campaign_id', batch);
        allLogs = allLogs.concat(logs || []);
      }

      // Đếm sent/failed thực tế từ send_logs
      const logCountsByCampaign = {};
      for (const log of allLogs) {
        if (!logCountsByCampaign[log.campaign_id]) logCountsByCampaign[log.campaign_id] = { sent: 0, failed: 0 };
        if (log.status === 'sent') logCountsByCampaign[log.campaign_id].sent++;
        else logCountsByCampaign[log.campaign_id].failed++;
      }

      // Group resend_ids theo campaign (chỉ log có resend_id — cho tracking events)
      const logsByCampaign = {};
      for (const log of allLogs) {
        if (!log.resend_id) continue;
      ...
      // 4. Auto-complete: kiểm tra campaigns bị paused mà thực tế đã gửi hết
      for (const camp of campaigns) {
        const counts = logCountsByCampaign[camp.id] || { sent: 0, failed: 0 };
        // Luôn fix sent_count từ logs (chính xác hơn campaign record)
        if (counts.sent > (camp.sent_count || 0)) camp.sent_count = counts.sent;
        if (counts.failed > (camp.failed_count || 0)) camp.failed_count = counts.failed;

        // Auto-complete cho campaigns paused HOẶC sending (bị kẹt) có logs
        if ((camp.status !== 'paused' && camp.status !== 'sending') || counts.sent === 0) continue;
        try {
          const totalContacts = await this.countContactsByLevelIds(camp.target_levels || []);
          if (totalContacts > 0 && (counts.sent + counts.failed) >= totalContacts) {
            // Tất cả đã gửi → tự động đánh dấu hoàn thành
            await this.updateCampaignStatus(camp.id, {
              status: 'completed', sent_count: counts.sent, failed_count: counts.failed,
            }).catch(() => {});
            camp.status = 'completed';
            console.log(`[auto-complete] Campaign ${camp.id} (${camp.name}): ${counts.sent} sent, ${counts.failed} failed / ${totalContacts} total → completed`);
          }
        } catch (_) { /* bỏ qua lỗi count */ }
      }

      // 5. Aggregate stats cho mỗi campaign
      return campaigns.map(camp => {
        ...
        return {
          ...camp,
          sent_count: Math.max(camp.sent_count || 0, counts.sent),
          failed_count: Math.max(camp.failed_count || 0, counts.failed),
          log_total: counts.sent + counts.failed,
          tracking: this._calcEventStats(events),
        };
      });
    },
    ...
    async getDashStats() {
      const [lv, ca, countMap, totalRes] = await Promise.all([
        ...
        sb.from('campaigns')
          .select('id, name, status, sent_count, failed_count, target_levels, sent_at')
          .neq('name', '__KILL_SWITCH__')        // ẩn row kill switch
    ...
    async _evalSegment(seg) {
      ...
      if (logic === 'or') {
        const conds = [];
        if (levelIds.length) conds.push(`level_ids.ov.{${levelIds.join(',')}}`);
