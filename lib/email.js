...
function render(content, contact) {
  return content
    ...
    .replace(/\{\{child_name\}\}/g, contact.child_name || '')
    .replace(/\{\{ten_con\}\}/g,    contact.child_name || '')
...
export async function sendOneEmail(campaign, contact, { resendKey, fromEmail } = {}) {
  ...
  const ctxData = { ...contact, level, company: contact.company || '', child_name: contact.child_name || '' };
...
export async function sendTestEmail({ to, subject, body_text, from_name }, { resendKey, fromEmail } = {}) {
  ...
  const resend  = getResend(key);
  const sample  = { name: 'Nguyễn Văn A', email: to, level: 'L1', company: 'UCMAS Vietnam', child_name: 'Bé Nguyễn Văn B' };
