import { getContactsByLevelIds, createCampaign, updateCampaignStatus, logSend, markLastSent } from '../lib/supabase.js';
import { sendCampaign } from '../lib/email.js';
import { err, allowCors } from './_utils.js';
...
export default async function handler(req, res) {
  ...
  let contacts;
  try {
    contacts = await getContactsByLevelIds(target_level_ids);
  ...
  // Server-Sent Events để stream tiến độ
  ...
  try {
    ...
    await Promise.all(results.map(r => logSend({
    ...
    if (sentIds.length) await markLastSent(sentIds);
    ...
    await updateCampaignStatus(campaign.id, { status: 'failed', sent_count: 0, failed_count: contacts.length });
