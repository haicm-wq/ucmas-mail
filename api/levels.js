import { getLevels, createLevel, deleteLevel, getContacts } from '../lib/supabase.js';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const [levels, contacts] = await Promise.all([getLevels(), getContacts()]);
      const countMap = {};
    ...
    try {
      ...
      ok(res, await createLevel({ name, color: color || '#5ba8ff', parent_id, description, sort_order }));
